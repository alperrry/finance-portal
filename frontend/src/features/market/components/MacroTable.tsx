...
export function MacroTable({ rows, sortConfig, onSort }: Props) {
    ...
    const typeLabel = (dataType: string) => {
        if (dataType === "INFLATION") return t("market.tables.macro.inflation");
        if (dataType === "DEPOSIT_RATE") return t("market.tables.macro.deposit");
        return dataType;
    };
    ...
    const COLUMNS: GridColDef<MacroObservationResponse>[] = [
        {
            field: "series",
            headerName: t("market.tables.macro.series"),
            flex: 1.4,
            minWidth: 160,
            renderCell: ({ row }) => (
        ...
        {
            field: "dataType",
            headerName: t("market.tables.macro.dataType"),
            flex: 0.8,
            minWidth: 100,
            renderCell: ({ row }) => (
        ...
        {
            field: "value",
            headerName: t("market.tables.macro.value"),
            flex: 0.9,
            minWidth: 100,
        ...
        {
            field: "monthlyChange",
            headerName: t("market.tables.macro.monthlyChange"),
            flex: 1,
            minWidth: 120,
        ...
        {
            field: "annualChange",
            headerName: t("market.tables.macro.yearlyChange"),
            flex: 1,
            minWidth: 120,
            renderCell: ({ row }) => {
        ...
        {
            field: "date",
            headerName: t("market.tables.macro.date"),
            flex: 0.9,
            minWidth: 100,
    ...
    return (
        <AppDataGrid<MacroObservationResponse>
            ...
            emptyMessage={t("market.tables.macro.empty")}
            rowHeight={52}
        />
    );
}
